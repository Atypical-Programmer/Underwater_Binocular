...
bool System::SaveMapPointsXYZ(const string &filename, const int minObservations)
{
    if (minObservations < 0)
    {
        cerr << "ERROR: minObservations must be non-negative." << endl;
        return false;
    }
    ...
    if (vpKFs.empty())
    {
        cerr << "ERROR: cannot save MapPoints because the active map has no keyframes." << endl;
        return false;
    }
    sort(vpKFs.begin(), vpKFs.end(), KeyFrame::lId);

    // SaveTrajectoryTUM uses this same transform to put the first keyframe at
    // the origin.  Applying its inverse to MapPoint world positions keeps the
    // exported point cloud in exactly the trajectory's coordinate frame.
    const Sophus::SE3f Two = vpKFs.front()->GetPoseInverse();
    const Sophus::SE3f Tnew_old = Two.inverse();

    vector<MapPoint*> vpMPs = mpAtlas->GetAllMapPoints();
    ofstream f(filename.c_str());
    if (!f.is_open())
    {
        cerr << "ERROR: cannot open MapPoint output file: " << filename << endl;
        return false;
    }

    f << "map_point_id,map_id,observations,x,y,z\n";
    size_t exported = 0;
    for (MapPoint* pMP : vpMPs)
    {
        if (!pMP || pMP->isBad() || pMP->Observations() < minObservations)
            continue;

        Eigen::Vector3f position = Tnew_old * pMP->GetWorldPos();
        if (!position.allFinite())
            continue;

        Map* pMap = pMP->GetMap();
        const unsigned long mapId = pMap ? pMap->GetId() : 0;
        f << pMP->mnId << ',' << mapId << ',' << pMP->Observations() << ','
          << setprecision(9) << position.x() << ',' << position.y() << ',' << position.z() << '\n';
        ++exported;
    }
    f.close();

    cout << "Saved " << exported << " MapPoints to " << filename << endl;
    return exported > 0;
}

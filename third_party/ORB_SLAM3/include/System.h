...
public:
    ...
    // Save the non-bad MapPoints from the active map in the same local metric
    // world frame used by SaveTrajectoryTUM().  The CSV is intentionally
    // simple so a later pass can attach colors from the source images.
    bool SaveMapPointsXYZ(const string &filename, const int minObservations = 2);
